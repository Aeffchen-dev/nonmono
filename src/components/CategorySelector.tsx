...
export function CategorySelector({ 
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-full max-w-[500px] h-[calc(100vh-200px)] md:h-[calc(100vh-130px)] bg-background border-0 p-0 overflow-hidden [&>button]:hidden" style={{ borderRadius: '16px' }}>
          ...
          <div className="flex-1 px-6 space-y-3 overflow-y-auto">
            {categories.map((category) => {
              ...
              return (
                <div 
                  key={category}
                  className={`flex items-center justify-between p-4 border-l-4 ${colorClasses} bg-[#161616] cursor-pointer`}
